...
# Use devcontainer.cmd on Windows/WSL
...
test_devcontainer() {
    ...
    if cd "$test_dir" && timeout 300 $DEVC_CMD build --workspace-folder . &>/tmp/devc-build-$$.log; then
        ...
        if timeout 60 $DEVC_CMD exec --workspace-folder . bash /workspaces/$(basename "$test_dir")/test.sh &>/tmp/devc-exec-$$.log; then
...
if [[ "$WSL_SUCCESS" == "1" ]]; then
    ...
    echo ""
    if [[ "$QUICK" != "true" ]] && [[ "$WIN_SUCCESS" != "1" ]]; then
        echo -e "${YELLOW}Note${NC}: /mnt/c/ location may have issues due to filesystem limitations"
        echo "Recommendation: Use WSL filesystem (~/) for BitBot projects"
    fi
    echo ""
    exit 0
else
