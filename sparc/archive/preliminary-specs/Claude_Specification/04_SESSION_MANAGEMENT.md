...
**Created**: 2025-10-17
**Last Updated**: 2025-10-20 (Added interactive session menu documentation)
...
## 7. Interactive Session Menu

### 7.1 BitBot Session Manager

**Purpose**: When user enters devcontainer, present interactive menu for session choices.
...
### 7.2 Session Choice Options

**Option 1: Resume detached tmux session**
```bash
# Show available tmux sessions
tmux list-sessions
# Output:
# work-main: 3 windows (created Thu Oct 17 10:00:00 2025) (attached)
# work-feature-x: 2 windows (created Thu Oct 17 11:00:00 2025)
...
**Use case**: User has existing tmux session from previous work, wants to continue.

**Option 2: Resume inactive AI session**
```bash
# Resume last Claude Code session
claude --resume

# Output:
# Resuming session from 2025-10-17 12:30:00...
# Last conversation: "Implementing feature X"
```

**Use case**: User left AI mid-conversation, wants to continue where they left off.

**Option 3: Start new AI session**
```bash
...
**Use case**: User wants fresh AI conversation for new task.

**Option 4: Exit to shell**
```bash
# Drop to bash prompt
...
**Use case**: User wants manual control, no AI assistance needed.

### 7.3 Implementation

**Script location**: `/opt/bitbot/bin/bitbot-session-menu.sh`
...
show_session_menu() {
    ...
    case "$choice" in
        1)
            ...
            if [[ -n "$session_name" ]]; then
                tmux attach -t "$session_name" || {
                    echo "Failed to attach to session: $session_name"
                    show_session_menu
                }
            else
                show_session_menu
            ...
            ;;
        2)
            # Option 2: Resume Claude session
...
**Integration points**:
- Called from `.bashrc` or `.zshrc` in devcontainer
- Triggered by `bitbot work` after container entry
- Triggered by `bitbot setup` after container entry
- Can be run manually: `bitbot-session-menu`

### 7.4 Auto-run Configuration

**Auto-run on container entry** (`.devcontainer/devcontainer.json`):
```json
{
  "postStartCommand": "/opt/bitbot/bin/bitbot-session-menu.sh",
  "remoteUser": "root"
}
```

**Conditional auto-run** (in `.bashrc`):
```bash
# Only show menu if interactive shell AND not already in tmux/claude
if [[ $- == *i* ]] && [[ -z "$TMUX" ]] && [[ -z "$CLAUDE_SESSION" ]]; then
    /opt/bitbot/bin/bitbot-session-menu.sh
fi
```

---

## 8. AI Agent Session Management
